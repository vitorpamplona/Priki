...
public class GoodLinkTransactionTest extends TestCase {

    public void testExecuteOnWiki() {
        Prevalence p = Prevalence.getInstance();
        GoodUserTransaction transaction = new GoodUserTransaction("user vitor");
        p.execute(transaction);
        
        Police police = p.getWiki().getPolice();
        
        assertTrue(!police.isBadLink("user vitor"));
    }
